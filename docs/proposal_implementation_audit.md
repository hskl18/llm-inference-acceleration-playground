...
| Fixed-prompt workloads | Plain-text and JSONL prompt loading, prompt-set fingerprinting, config-relative prompt paths, prompt text excluded from benchmark metadata | `test_cli_latency_benchmark_accepts_fixed_prompt_file`, `test_cli_sweep_accepts_prompt_workload` |
