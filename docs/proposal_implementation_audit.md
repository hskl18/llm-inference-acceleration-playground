...
| Quantization comparison | One endpoint per mode with distinct-endpoint enforcement, served-model checks, mode support labeling, per-mode benchmark metadata, baseline exact-match and perplexity-delta quality scoring | `test_quantization_comparison.py`, `test_quantization_sanity.py` |
...
| Speculative decoding study | Leviathan et al. 2023 closed-form speedup model, acceptance curves, measured acceptance from vLLM `/metrics`, baseline comparison artifacts, method notes | `test_speculative_analysis.py`, `test_vllm_server.py` |
